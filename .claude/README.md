...
`enabledMcpjsonServers` ב־`.claude/settings.json` מאשר מראש את שלושת השרתים מ־`.mcp.json`,
כך שלא תופיע בקשת אישור בתחילת סשן. שרת חדש שיתווסף ל־`.mcp.json` בעתיד עדיין
יבקש אישור — צריך להוסיף את שמו לרשימה במפורש.
